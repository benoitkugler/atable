...
func ScanMenuRecette(row *sql.Row) (MenuRecette, error) { return scanOneMenuRecette(row) }

// SelectAll returns all the items in the menu_recettes table.
func SelectAllMenuRecettes(db DB) (MenuRecettes, error) {
	rows, err := db.Query("SELECT * FROM menu_recettes")
	if err != nil {
		return nil, err
	}
...
func ScanMenuRecettes(rs *sql.Rows) (MenuRecettes, error) {
	...
	for rs.Next() {
		item, err = scanOneMenuRecette(rs)
		if err != nil {
			return nil, err
		}
		structs = append(structs, item)
...
// Delete the link MenuRecette from the database.
// Only the foreign keys IdMenu, IdReceipe fields are used in 'item'.
func (item MenuRecette) Delete(tx DB) error {
	_, err := tx.Exec(`DELETE FROM menu_recettes WHERE IdMenu = $1 AND IdReceipe = $2;`, item.IdMenu, item.IdReceipe)
	return err
}

// ByIdMenu returns a map with 'IdMenu' as keys.
func (items MenuRecettes) ByIdMenu() map[IdMenu]MenuRecettes {
	out := make(map[IdMenu]MenuRecettes)
	for _, target := range items {
		out[target.IdMenu] = append(out[target.IdMenu], target)
	}
	return out
...
// contained in this link table.
// They are not garanteed to be distinct.
func (items MenuRecettes) IdMenus() []IdMenu {
	out := make([]IdMenu, len(items))
	for index, target := range items {
		out[index] = target.IdMenu
	}
...
func SelectMenuRecettesByIdMenus(tx DB, idMenus_ ...IdMenu) (MenuRecettes, error) {
	rows, err := tx.Query("SELECT * FROM menu_recettes WHERE idmenu = ANY($1)", IdMenuArrayToPQ(idMenus_))
	if err != nil {
		return nil, err
	}
	return ScanMenuRecettes(rows)
...
func DeleteMenuRecettesByIdMenus(tx DB, idMenus_ ...IdMenu) (MenuRecettes, error) {
	rows, err := tx.Query("DELETE FROM menu_recettes WHERE idmenu = ANY($1) RETURNING *", IdMenuArrayToPQ(idMenus_))
	if err != nil {
		return nil, err
	}
	return ScanMenuRecettes(rows)
...
func (items MenuRecettes) ByIdReceipe() map[IdReceipe]MenuRecettes {
	out := make(map[IdReceipe]MenuRecettes)
	for _, target := range items {
...
// contained in this link table.
// They are not garanteed to be distinct.
func (items MenuRecettes) IdReceipes() []IdReceipe {
...
func SelectMenuRecettesByIdReceipes(tx DB, idReceipes_ ...IdReceipe) (MenuRecettes, error) {
	rows, err := tx.Query("SELECT * FROM menu_recettes WHERE idreceipe = ANY($1)", IdReceipeArrayToPQ(idReceipes_))
	if err != nil {
		return nil, err
	}
	return ScanMenuRecettes(rows)
...
func DeleteMenuRecettesByIdReceipes(tx DB, idReceipes_ ...IdReceipe) (MenuRecettes, error) {
	rows, err := tx.Query("DELETE FROM menu_recettes WHERE idreceipe = ANY($1) RETURNING *", IdReceipeArrayToPQ(idReceipes_))
	if err != nil {
		return nil, err
	}
	return ScanMenuRecettes(rows)
